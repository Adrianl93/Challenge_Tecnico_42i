...
export default function ModalFindPair({setModalOne, modalOne}) {
  // props validation
  ModalFindPair.propTypes = {
    setModalOne: PropTypes.func.isRequired,
    modalOne: PropTypes.bool.isRequired,
  };
  ...
  function handleArray(e) {
    ...
    // creates a constant in which the comma is replaced
    // by an empty string space ' '
  ...
  const findPairWithSum = (num, targetSum) => {
    ...
    for (let i = 0; i < num.length; i++) {
      // aux take value from targetsum and
      // substracts the value of num in the current loop
      const aux = targetSum - num[i];

      // It looks to see if the aux value is in seenNums
      // and if it exists, it pushes it to output and
      // the current value of num[i] too.
      if (seenNums.has(aux)) {
        output.push(aux);
        output.push(num[i]);
        return output;
      }
      // If aux is not in seenNums it add the current
      // value of num[i] to this Set
      seenNums.add(num[i]);
    }
    // Finally it return the array with the
    // two numbers that sum the value of targetSum
    return output;
  };
        ...
        <div className="containerModal">
          ...
          {results.length > 0 ? (
            <h4 className="results">
              The numbers {results[0]} and {results[1]} sums {input.targetSum}
            </h4>
          ) : (
            resultsH2 && (
              <h4 className="results">
                Sorry, no pair found for the target sum
              </h4>
            )
          )}
        </div>
